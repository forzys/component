...
`),H=Math.max(L-3,0),P=Math.min(z.length,L+3),x=C(E),D=z.slice(H,P).map(function(F,R){var N=R+H+1;return(N==L?" >> ":"    ")+N+"| "+F}).join(`
...
`;return e=e.replace(/^[ ]{0,2}([ ]?\*[ ]?){3,}[ \t]*$/gm,a),e=e.replace(/^[ ]{0,2}([ ]?-[ ]?){3,}[ \t]*$/gm,a),e=e.replace(/^[ ]{0,2}([ ]?_[ ]?){3,}[ \t]*$/gm,a),e=C(e),e=P(e),e=N(e),e=k(e),e=W(e,u),e}function g(e){return e=D(e),e=f(e),e=pe(e),e=M(e),e=l(e),e=ce(e),e=e.replace(/~P/g,"://"),e=ee(e),e=R(e),e=e.replace(/  +\n/g,` <br>
...
`}function D(e){return e=e.replace(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm,function(u,a,s,p,b){var h=p;return h=h.replace(/^([ \t]*)/g,""),h=h.replace(/[ \t]*$/g,""),h=F(h),h=h.replace(/:\/\//g,"~P"),a+"<code>"+h+"</code>"}),e}function F(e){return e=e.replace(/&/g,"&amp;"),e=e.replace(/</g,"&lt;"),e=e.replace(/>/g,"&gt;"),e=O(e,"*_{}[]\\",!1),e}function R(e){return e=e.replace(/([\W_]|^)(\*\*|__)(?=\S)([^\r]*?\S[\*_]*)\2([\W_]|$)/g,"$1<strong>$3</strong>$4"),e=e.replace(/([\W_]|^)(\*|_)(?=\S)([^\r\*_]*?\S)\2([\W_]|$)/g,"$1<em>$3</em>$4"),e}function N(e){return e=e.replace(/((^[ \t]*>[ \t]?.+\n(.+\n)*\n*)+)/gm,function(u,a){var s=a;return s=s.replace(/^[ \t]*>[ \t]?/gm,"~0"),s=s.replace(/~0/g,""),s=s.replace(/^[ \t]+$/gm,""),s=w(s),s=s.replace(/(^|\n)/g,"$1  "),s=s.replace(/(\s*<pre>[^\r]+?<\/pre>)/gm,function(p,b){var h=b;return h=h.replace(/^  /mg,"~0"),h=h.replace(/~0/g,""),h}),x(`<blockquote>
`+s+`
